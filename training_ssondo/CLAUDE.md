...
- `DATA` - root data directory (contains `AudioSet/` and `teachers_knowledge/`)
- `OUTPUTS` - output directory (clustering results, trained models)
...
2. **`extract_teachers_knowledge/`** - Runs audio through teacher models (MATPAC or M2D), saves embeddings as `.npz` files. Teacher models are wrapped in `models_wrappers.py` with a common `ModelWrapper` interface.

3. **`cluster_teachers_embeddings/`** - Three-stage pipeline: train MiniBatchKMeans on embeddings, predict cluster labels for all samples, evaluate with silhouette/Calinski-Harabasz/Davies-Bouldin metrics.
...
4. **`knowledge_distillation_training/`** - The main training module. Key files:
   ...
   - `data_pipeline.py` - builds preprocessing chain (SliceAudio -> LogMelSpectrogram -> Normalize), datasets, samplers, dataloaders
