...
class LocalViT(timm.models.vision_transformer.VisionTransformer):
  ...
  def forward_encoder(self, x):
    ...
    for blk in self.blocks:
      x = blk(x)
      layer_results.append(x.unsqueeze(dim=1))
    x = self.norm(x)
    layer_results[-1] = x.unsqueeze(dim=1)

    x = torch.cat(layer_results, dim=1)
...
def parse_sizes_by_name(name):
  ...
  input_str, patch_str, sr = params[0], params[1], params[2] if len(
      params) > 2 else '16k'
...
def get_backbone(args, weight_file):
  args.input_size, args.patch_size, args.sr, args.model = parse_sizes_by_name(
      Path(weight_file).parent.name)
...
class PortableM2D(torch.nn.Module):
  def __init__(self, weight_file, num_classes=None, freeze_embed=False, flat_features=None):
    ...
    n_stack_feature = 1 if self.cfg.flat_features else (
        self.cfg.input_size[0] // self.cfg.patch_size[0])
    # 768 if flat_features else 768*5=3840
    self.cfg.feature_d = d * n_stack_feature
    # Create head.
    ...
    if self.cfg.flat_features:
      ...
      for i in range(n_chunk):
        ...
        if average_per_time_frame:
          emb = rearrange(emb, 'b l (f t) d -> b l t d f',
                          l=emb.shape[1], f=patch_fbins, d=embed_d).mean(-1)
      ...
      for i in range(n_chunk):
        emb = self.backbone.forward_encoder(
            x[..., i*unit_frames:(i+1)*unit_frames])
        emb = emb[..., 1:, :]
        emb = rearrange(emb, 'b n (f t) d -> b n t (f d)',
...
if __name__ == "__main__":
  import os
  model = PortableM2D(
      "/home/auquelennec/Téléchargements/m2d_vit_base-80x608p16x16-221006-mr7/checkpoint-300.pth"
  )
