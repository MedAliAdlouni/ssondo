...
class PortableM2D(torch.nn.Module):
    def __init__(
        self, weight_file, num_classes=None, freeze_embed=False, flat_features=None
    ):
        super().__init__()
        self.cfg = Config()
        ...
        if (
            ...
            and "module.head.mlp.mlp.0.weight" in checkpoint
            and checkpoint["module.head.mlp.mlp.0.weight"].shape[-1] == d
        ):
    ...
    def encode_lms(self, x, average_per_time_frame=False):
        ...
        if self.cfg.flat_features:
            ...
            for i in range(n_chunk):
                ...
                emb = emb[..., 1:, :]
                emb = rearrange(
                    emb, "b n (f t) d -> b n t (f d)", f=patch_fbins, d=embed_d
                )
