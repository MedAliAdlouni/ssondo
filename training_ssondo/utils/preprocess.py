...
class SliceAudio(nn.Module):
    ...
    def __init__(self, sr=16000, window_length=1, step_size=1, add_last=False) -> None:
...
if __name__ == "__main__":
    example = torch.randn((1, 320000))

    slicer = SliceAudio(sr=32000, window_length=2, step_size=2)
    logmel = LogMelSpectrogram(
        sample_rate=32000, win_length=0.025, hop_length=0.01, n_mels=128
    )

    preprocess = nn.Sequential(slicer, logmel)

    melspec = preprocess(example)
