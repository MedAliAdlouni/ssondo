...
from training_ssondo.utils.student_models.pooling_layers import (
    AttentionPooling,
    MeanPooling,
)
    ...
    def __init__(
        ...
        if pooling == "attention":
            self.pooling = AttentionPooling(
                input_size=n_classes, activation=activation_att
            )
        elif pooling == "mean":
        ...
        print("Number of trainable parameters - Linear Classifier: ", self.n_parameters)
...
class MLPClassifer(nn.Module):
    ...
    def __init__(
        ...
        self.n_parameters = count_parameters(self)
        print("Number of trainable parameters - MLP Classifier: ", self.n_parameters)
...
class RNNClassifer(nn.Module):
    ...
    def __init__(
        ...
        self.n_parameters = count_parameters(self)
        print("Number of trainable parameters - RNN Classifier: ", self.n_parameters)
...
if __name__ == "__main__":
    linear_att_head = LinearClassifer(
        emb_size=960, n_classes=527, pooling="attention", activation_att="sigmoid"
    )

    linear_mean_head = LinearClassifer(emb_size=960, n_classes=527, pooling="mean")

    mlp_att_head = MLPClassifer(
        emb_size=960,
        n_classes=527,
