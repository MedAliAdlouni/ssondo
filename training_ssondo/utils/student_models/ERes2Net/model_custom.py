...
class ERes2Net(nn.Module):
    def __init__(
        ...
        if self.add_layer:
            self.stats_dim = int(feat_dim / 16) * m_channels * 4
        else:
        ...
        self.layer3 = self._make_layer(
            block_fuse, m_channels * 2, num_blocks[2], stride=2
        )
        self.layer4 = self._make_layer(
            block_fuse, m_channels * 2, num_blocks[3], stride=2
        )
        if self.add_layer:
            self.layer5 = self._make_layer(
                block_fuse, m_channels * 4, num_blocks[4], stride=1
            )
            self.layer6 = self._make_layer(
                block_fuse, m_channels * 8, num_blocks[5], stride=2
            )
        ...
        if self.add_layer:
            ...
            self.layer5_downsample = nn.Conv2d(
                m_channels * 8,
                m_channels * 16,
                kernel_size=3,
                padding=1,
        ...
        self.fuse_mode1234 = AFF(channels=m_channels * 4)
        if self.add_layer:
            self.fuse_mode12345 = AFF(channels=m_channels * 8)
            self.fuse_mode123456 = AFF(channels=m_channels * 16)
    ...
    def forward(self, x):
        ...
        if self.add_layer:
            ...
            # Layer123 and layer4 Fuse
            fuse_out12345_downsample = self.layer5_downsample(fuse_out12345)
            fuse_out123456 = self.fuse_mode123456(out2, fuse_out12345_downsample)

            emb = self.pool(fuse_out123456)
...
if __name__ == "__main__":
    ...
    model = ERes2Net(
        m_channels=16,
        feat_dim=128,
        num_blocks=[2, 2, 3, 4, 2, 3],
        pooling_func="TAP",
        add_layer=True,
    )

    class_head = LinearClassifer(
        emb_size=model.emb_size, n_classes=527, pooling="mean", activation_att=""
    )
    ...
    optimizer = torch.optim.AdamW(params=full_model.parameters(), lr=0.001)

    # Simulating an input of a batch of 64 mel spectrogram of 98 time steps
    # and 128 frequency bins
    audio_example = torch.randn(size=(12, 1, 128, 998))
