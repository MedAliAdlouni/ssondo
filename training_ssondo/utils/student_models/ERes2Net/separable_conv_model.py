"""ERes2Net code with Separable Conv"""
...
class ERes2Net(nn.Module):
    def __init__(
        ...
        self.separable_conv1 = separable_conv1
        ...
        if self.separable_conv1:
            self.conv1 = SeparableConv2d(
                1, m_channels, kernel_size=3, stride=1, padding=1
            )
        else:
        ...
        if self.add_layer:
            self.layer5 = self._make_layer(
                block_fuse, m_channels * 2, num_blocks[4], stride=2
            )

        # Downsampling module for each layer
        self.layer1_downsample = SeparableConv2d(
            m_channels * 2, m_channels * 4, kernel_size=3, stride=2, padding=1
        )
        self.layer2_downsample = SeparableConv2d(
            m_channels * 4, m_channels * 8, kernel_size=3, padding=1, stride=2
        )
        self.layer3_downsample = SeparableConv2d(
            m_channels * 8, m_channels * 16, kernel_size=3, padding=1, stride=2
        )
        if self.add_layer:
            self.layer4_downsample = SeparableConv2d(
                m_channels * 16, m_channels * 4, kernel_size=3, padding=1, stride=2
            )
        ...
        print(
            "Number of trainable parameters - ERes2Net_SeparableConv: ",
            self.n_parameters,
        )
...
if __name__ == "__main__":
    ...
    # Embedding_size is the parameter to change the size of the embedding.
    # feat_dim is the parameter that correspond to the number of frequency bins
    model = ERes2Net(
        m_channels=16, feat_dim=128, num_blocks=[3, 4, 6, 3, 3], add_layer=True
    )
    ...
    full_model = ModelWrapper(model=model, classification_head=class_head)

    full_model = full_model.to("cuda")

    loss_fn = torch.nn.BCELoss()

    optimizer = torch.optim.SGD(params=full_model.parameters(), lr=0.001)

    # Simulating an input of a batch of 64 mel spectrogram of 160 time steps
    # and 128 frequency bins
    audio_example = torch.randn(size=(4, 10, 128, 98))
    audio_example = audio_example.to("cuda")
    target = torch.rand((4, 527)) - 0.2 > 0.5
