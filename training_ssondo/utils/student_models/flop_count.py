...
This module provides a function to calculate the FLOPs of a CNN model.
The code is taken from the following repository:
https://github.com/fschmid56/EfficientAT/blob/9a6efb39f91aa133ae7ff936cf57752b04082c9e/helpers/flop_count.py
"""
...
def count_macs(model, spec_size):
  ...
  print("Relative Share of Convolutional Layers: {:.2f}".format((sum(list_conv2d) / total_macs)))  # nopep8
  print("Relative Share of Linear Layers: {:.2f}".format(sum(list_linear) / total_macs))  # nopep8
  print("Total MACs (multiply-accumulate operations in Billions): {:.2f}".format(total_macs / 10**9))  # nopep8
  print("********************************************************************")
  return total_macs
...
def count_macs_transformer(model, spec_size):
  ...
  def attention_hook(self, input, output):
    # here we only calculate the attention macs; linear layers are processed
    # in linear_hook
    batch_size, seq_len, embed_size = input[0].size()
  ...
  print("Relative Share of Convolutional Layers: {:.2f}".format((sum(list_conv2d) / total_macs)))  # nopep8
  print("Relative Share of Linear Layers: {:.2f}".format(sum(list_linear) / total_macs))  # nopep8
  print("Relative Share of Attention Layers: {:.2f}".format(sum(list_att) / total_macs))  # nopep8
  print("Total MACs (multiply-accumulate operations in Billions): {:.2f}".format(total_macs / 10**9))  # nopep8
  print("********************************************************************")
  return total_macs
