...
class AudiosetDataset(Dataset):
    ...
    def __init__(
        self, subset: str, sr: int = 16000, audio_duration: Optional[int] = None
    ) -> None:

        audioset = AudioSet(root_dir=os.path.join(DATA, "AudioSet"))

        pdf = audioset._pdf_metadata.copy(deep=True)
        pdf = self.__init_aggregated_labels(
            pdf
        )  # Assign the returned aggregated DataFrame
    ...
    def __init_aggregated_labels(self, pdf: pd.DataFrame) -> pd.DataFrame:
        ...
        metadata_file_path = os.path.join(DATA, "AudioSet", "metadata.csv")

        cache_file = os.path.join(
            os.path.dirname(metadata_file_path), "metadata_aggregated_cache.pkl"
        )
        use_cache = False
        ...
        if os.path.exists(cache_file):
            try:
                ...
                if (
                    isinstance(pdf_aggregated, type(pdf))
                    and "file_id" in pdf_aggregated.columns
                ):
                    use_cache = True
                    print("Labels loaded from cache.")
                    return pdf_aggregated
        ...
        if not use_cache:
            print(
                "Aggregating labels..."
            )  # Aggregate label and label_idx into lists by file_id
            aggregated = (
                pdf.groupby("file_id")
                .agg({"label": list, "label_idx": list})
                .reset_index()
            )
    ...
    def load_set(
        self, pdf: pd.DataFrame, subset: str
        ...
        if self.audio_duration is None:
            ...
            metadata_df = metadata_df[
                metadata_df["duration"] == AUDIOSET_CLIP_DURATION_S
            ]
    ...
    def central_crop_or_pad_audio_tensor(
        ...
        # Pad the audio tensor if it is shorter than the desired duration
            ...
            audio_tensor = torch.nn.functional.pad(audio_tensor, pad=(0, padding_size))
    ...
    def load_audio_tensor(self, file_path: str) -> torch.Tensor:
        ...
        if self.audio_duration is not None:
            ...
            expected_samples = int(AUDIOSET_CLIP_DURATION_S * self.sr)
            if audio_tensor.shape[-1] != expected_samples:
                audio_tensor = self.central_crop_or_pad_audio_tensor(
                    audio_tensor,
                    audio_duration=int(AUDIOSET_CLIP_DURATION_S),
                    sample_rate=self.sr,
                )
