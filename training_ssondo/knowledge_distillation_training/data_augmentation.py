...
class Mixup(nn.Module):
    ...
    def __init__(self, alpha=0.3) -> None:
    ...
    def forward(self, sources, labels=None) -> tuple:
        """
        ...
        mixed_sources = sources * lam.reshape(lam_shape) + sources[rn_indices] * (
            1.0 - lam.reshape(lam_shape)
        )

        if labels is not None:
            lam_shape = (bs,) + tuple([1] * (labels.ndim - 1))
            mixed_labels = labels * lam.reshape(lam_shape) + labels[rn_indices] * (
                1.0 - lam.reshape(lam_shape)
            )
        else:
            mixed_labels = None
