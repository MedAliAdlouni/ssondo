...
class KnowledgeDistillationSystem(pl.LightningModule):
  ...
  def training_step(self, train_batch, batch_idx):
    ...
    if self.conf["data_augmentation"]["mixup"]:
      ...
      bs = x.size(0)
      rn_indices, lam = None, None  # Just to have variables defined
    ...
    if self.kd_loss_func is not None:
      if isinstance(self.kd_loss_func, torch.nn.KLDivLoss):
        ...
        kd_loss = self._kd_loss_default(
            y_hat, y_teacher, None, None, x.size(0))
    else:
  ...
  def validation_step(self, val_batch, batch_idx):
    ...
    self.log("val/pred_loss", pred_loss, on_epoch=True,
             prog_bar=True, sync_dist=True)
    self.log("val/kd_loss", kd_loss, on_epoch=True,
             prog_bar=True, sync_dist=True)
  ...
  def on_validation_epoch_end(self) -> None:
    ...
    # compute the mAP
    all_y = self.all_gather(all_y).reshape(-1, all_y.shape[-1]).cpu().float().numpy()  # nopep8
    all_y_hat = self.all_gather(all_y_hat).reshape(-1, all_y_hat.shape[-1]).cpu().float().numpy()  # nopep8

    try:
      average_precision = metrics.average_precision_score(all_y, all_y_hat, average=None)  # nopep8

    except ValueError:
      average_precision = np.array([np.nan] * self.conf["dataset"]["n_classes"])  # nopep8
  ...
  def _kd_loss_default(self, y_hat, y_teacher, rn_indices, lam, bs):
    if self.conf["data_augmentation"]["mixup"]:
      kd_loss = self.kd_loss_func(
          y_hat, y_teacher).mean(dim=1) * lam.reshape(bs)
      kd_loss += self.kd_loss_func(y_hat,
                                   y_teacher[rn_indices]).mean(dim=1) * (1 - lam.reshape(bs))
    else:
  ...
  def _kd_loss_kl_div(self, y_hat, y_teacher, rn_indices, lam, bs):
    ...
    if self.conf["data_augmentation"]["mixup"]:
      kd_loss = self.kd_loss_func(
          y_hat_log_prob, y_teacher_prob).mean(dim=1) * lam.reshape(bs)
      kd_loss += self.kd_loss_func(y_hat_log_prob,
                                   y_teacher_prob[rn_indices]).mean(dim=1) * (1 - lam.reshape(bs))
    else:
  ...
  def kd_loss_real_contrastive(self, y_hat, y_teacher):
      ...
      logits = torch.matmul(y_hat, y_teacher.T) / self.conf["knowledge_distillation"]["temperature"]
  ...
  def kd_loss_contrastive_neg_clusters_only(self, y_hat, y_teacher, cluster_labels):
    ...
    same_cluster_mask = (cluster_labels_expanded ==
                         cluster_labels_expanded.T).float()
  ...
  def _kd_loss_cluster_aware_contrastive(self, y_hat, y_teacher, cluster_labels):
    ...
    same_cluster_mask = (cluster_labels_expanded ==
                         cluster_labels_expanded.T).float()
    diagonal_mask = torch.eye(cluster_labels.size(0),
    ...
    if valid_samples.sum() > 0:
      loss_st = -torch.log(pos_exp_st[valid_samples] /
                           (pos_exp_st[valid_samples] + neg_exp_st[valid_samples]))
      loss_ts = -torch.log(pos_exp_ts[valid_samples] /
  ...
  def hybrid_cluster_contrastive_loss(self, y_hat, y_teacher, cluster_labels, rn_indices=None, lam=None):
    """
    Hybrid KD loss:
    ...
    gamma = self.conf["knowledge_distillation"]["loss_params"]["cross_cluster_weight"]

    total_loss = alpha * alignment_loss + beta * \
        contrastive_loss + gamma * cross_cluster_loss
  ...
  def _cross_cluster_teacher_guidance_loss(self, y_hat, y_teacher, cluster_labels):
    ...
    for cluster_id in unique_clusters:
      ...
      student_centroid = F.normalize(
          student_centroid.unsqueeze(0), dim=1).squeeze(0)
      teacher_centroid = F.normalize(
          teacher_centroid.unsqueeze(0), dim=1).squeeze(0)
    ...
    student_centroids = torch.stack(
        student_centroids)  # [n_clusters, embedding_dim]
    teacher_centroids = torch.stack(
        teacher_centroids)  # [n_clusters, embedding_dim]
