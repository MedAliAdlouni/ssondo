...
class AudiosetDatasetKD(Dataset):
  ...
  def __init__(
    ...
    if self.cluster_labels_path:
      ...
      if "lustre" in os.environ.get("DATA", ""):
        ...
        cluster_labels['audio_id'] = cluster_labels['audio_id'].str.replace(
            old_path, new_path, regex=False)

      # Normalize audio_id paths to match normalized file paths (Windows compatibility)
      cluster_labels['audio_id'] = cluster_labels['audio_id'].apply(
          lambda x: os.path.normpath(x) if isinstance(x, str) else x)

      print(self.cluster_labels_path)
  ...
  def __getitem__(self, index):
    ...
    if self.teacher_knowledge_path is None:
      ...
      teacher_knowledge = np.load(
          os.path.join(self.teacher_knowledge_path,
                       f"{self.subset}",
                       f"{filename_without_ext}.npz"),
      )

      teacher_preds = torch.from_numpy(teacher_knowledge["embed"])  # shape : (1, emb_dim) # nopep8
    ...
    if self.cluster_labels_path is None:
      print("WARNING : cluster_labels_path is None")
      ...
      if file_id in self.id_to_cluster:
        cluster_label = torch.tensor(self.id_to_cluster[file_id], dtype=torch.long)
      else:
        ...
        if file_id_normalized in self.id_to_cluster:
          cluster_label = torch.tensor(self.id_to_cluster[file_id_normalized], dtype=torch.long)
        else:
          # If still not found, use default cluster 0 and warn
          print(f"Warning: Cluster label not found for file_id: {file_id}, using default cluster 0")
          cluster_label = torch.tensor(0, dtype=torch.long)
  ...
  def load_subset(self, pdf, subset):
    ...
    print(f"  {subset} subset: {initial_count} files in metadata, {final_count} files exist ({final_count/initial_count*100:.1f}%)")
