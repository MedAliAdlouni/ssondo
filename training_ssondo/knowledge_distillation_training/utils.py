...
def get_ft_weighted_sampler_SSL(
    train_dataset, num_samples: int = 100000, replacement=False, generator=None
...
def get_ft_weighted_sampler(
    ...
    samples_weights = get_ft_cls_balanced_sample_weights(pdf, num_classes)
...
def get_ft_cls_balanced_sample_weights(
    ...
    per_class_weights = 1000.0 / per_class  # shape: (1, num_classes)
    ...
    for i, label_idx in enumerate(pdf["label_idx"]):
        if isinstance(label_idx, (list, np.ndarray)):
            # Multi-label case
            indices = torch.tensor(label_idx, dtype=torch.long)
            sample_weights = per_class_weights[
                0, indices
            ]  # Get weights for this sample's classes
