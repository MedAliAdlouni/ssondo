...
def main(conf) -> None:
    ...
    print(f"Embedding dimension: {train_dataset[0][1].shape[-1]}")

    # Use DataLoader to stream dataset in batches
    dataloader = DataLoader(train_dataset, batch_size=conf["batch_size"], shuffle=True)
    ...
    if conf["use_fit"]:
        ...
        for i, (_, embeddings) in enumerate(
            tqdm(dataloader, desc="Processing batches for clustering", unit="batch")
        ):
