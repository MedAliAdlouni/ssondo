...
def plot_inertia(inertia_history, conf) -> None:
  ...
  plt.plot(range(1, len(inertia_history) + 1),
           inertia_history, marker='o', linestyle='-')
...
def sample_embeddings_and_labels(train_dataset, df, sample_size) -> Tuple[np.ndarray, np.ndarray]:
...
def compute_clustering_metrics(train_dataset, df, conf) -> dict:
  ...
  # sampled_labels = sample_embeddings_and_labels(train_dataset, df, sample_size)
...
def save_umap_data(train_dataset, df, conf) -> None:
  ...
  # def plot_tsne(train_dataset, df, conf):
