...
from .config import common_parameters
...
class TeacherKnowledgeDataset(Dataset):
  ...
  def load_subset(self, pdf, subset) -> Tuple[pd.DataFrame, np.ndarray]:
    ...
    if subset == "train":
      ...
      metadata_df_train = pdf[pdf["set"].isin(["unbalanced_train", "balanced_train"])]
      metadata_df_eval = pdf[pdf["set"] == "eval"]
      metadata_df = pd.concat([metadata_df_eval, metadata_df_train], ignore_index=True)
  ...
  def filter_existing_files(self, metadata_df, file_path_list, subset, teacher_knowledge_path) -> Tuple[pd.DataFrame, np.ndarray]:
    """
    ...
    print(f"Filtering {len(metadata_df)} samples to check for existing teacher knowledge files...")
    
    for idx, file_path in enumerate(file_path_list):
      file_name = os.path.basename(file_path).replace(".wav", ".npz")
      
      if subset == "all":
        full_path_train = os.path.join(teacher_knowledge_path, "train", file_name)
    ...
    print(f"Found {len(existing_indices)} existing files out of {len(metadata_df)} samples.")
  ...
  def __getitem__(self, index: int) -> Tuple[str, torch.Tensor]:
    ...
    if self.subset == "all":
      full_path_train = os.path.join(self.teacher_knowledge_path, "train", file_name)
      full_path_eval = os.path.join(self.teacher_knowledge_path, "eval", file_name)
      ...
      full_path = os.path.join(self.teacher_knowledge_path, self.subset, file_name)
